"""
给 data/audio/ 下的所有音频做低通滤波，输出到 data/audio_lowpass_{XXXX}Hz/

特点
----
- 用 cutoff (Hz) 控制低通截止频率，保留低于该频率的成分
...
1) 单个 cutoff（最常用）：
   ...
   输出到: data/audio_lowpass_4000Hz/

2) 扫多个 cutoff 做 robustness 实验：
       for cutoff in 8000 4000 2000 1000; do
           python src/low_pass.py --cutoff $cutoff
       done

3) 在 notebook 里改 AUDIO_DIR 即可使用：
       AUDIO_DIR = os.path.join(PROJECT_DIR, 'data', 'audio_lowpass_4000Hz')

参数说明
--------
--cutoff        低通截止频率（Hz）。常用: 8000, 4000, 2000, 1000。数值越小高频损失越强。
--order         Butterworth 滤波器阶数，默认 5。
--clean-dir     干净音频目录，默认 data/audio/。
--out-dir       输出目录，默认 data/audio_lowpass_{cutoff}Hz/。
...
# 2. 核心函数：低通滤波
# =========================
def low_pass_filter(signal: np.ndarray, sr: int, cutoff_hz: float, order: int = 5) -> np.ndarray:
    """
    对 signal 做低通滤波，保留低于 cutoff_hz 的频率成分。
...
def low_pass_dataset(
    ...
    for fname in tqdm(filenames, desc=f"Low-pass filtering @ cutoff={cutoff_hz:g}Hz"):
        ...
        try:
            filtered = low_pass_filter(signal, sr=sr, cutoff_hz=cutoff_hz, order=order)
        except Exception as e:
...
# =========================
def main():
    parser = argparse.ArgumentParser(description="Apply low-pass filtering to all audio files.")
    parser.add_argument("--cutoff", type=float, default=4000.0,
                        help="低通截止频率 (Hz)。常用值: 8000, 4000, 2000, 1000。")
    parser.add_argument("--order", type=int, default=5,
                        help="Butterworth 滤波器阶数，默认 5。")
    parser.add_argument("--clean-dir", type=str, default=CLEAN_AUDIO_DIR,
                        help="干净音频文件夹。")
    parser.add_argument("--out-dir", type=str, default=None,
                        help="输出文件夹，默认 data/audio_lowpass_{cutoff}Hz。")
    ...
    out_dir = args.out_dir or os.path.join(
        PROJECT_DIR, "data", f"audio_lowpass_{cutoff_label}Hz"
    )
